...
import {
  ...
  getPublicWrcCase,
  submitWrcCaseResponse,
} from '../lib/resources/weeklyRevenueClosurePublic'

export function WeeklyRevenueResponsePortalPage() {
  ...
  const {
    ...
    queryKey: ['weekly-revenue-public-case', token],
    queryFn: () => getPublicWrcCase(token),
    retry: false,
  })
  ...
  async function handleSubmit(fields: WrcResponseFields, evidence: File) {
    const result = await submitWrcCaseResponse(token, fields, evidence)
    queryClient.invalidateQueries({ queryKey: ['weekly-revenue-public-case', token] })
    return result
  }
      ...
      <div className="w-full max-w-lg">
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          <p className="mt-3 text-sm font-medium text-[color:var(--color-np-deep-blue)]">
            Weekly Revenue Closure — Response Required
          </p>
        </div>
        ...
        {caseData && (
          <WrcCaseCard caseData={caseData} centerDirectory={centerDirectory} onSubmit={handleSubmit} />
        )}
